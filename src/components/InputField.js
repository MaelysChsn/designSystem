...
function InputField({label, type, value, onChange, _icon, style, placeholder}) {
    if (type === "password") {
    ...
    if (type === "password") {
        return (
            <div
                className={`${style.input_icon_container} ${style.input_container} ${isActive ? style.input_icon_container_focus : ''}`}>
        ...
        return (
            <div
                ...
                <input placeholder={placeholder} type={type} value={value} onChange={onChange} onClick={onClick}
                       onBlur={offClick}/>
            </div>
        );
    }
